...
namespace DH2323_Project
{
    internal static class ModelLoader
    {
        ...
        public unsafe static ObjData LoadBinModel(string path)
        {
            BinaryReader reader = new BinaryReader(File.OpenRead(path));

            int vertexCount = reader.ReadInt32();
            Vertex[] vertices = new Vertex[vertexCount];
            byte[] bytes = reader.ReadBytes(vertexCount * sizeof(Vertex));
            MemoryMarshal.Cast<byte, Vertex>(bytes).CopyTo(vertices);

            int objCount = reader.ReadInt32();
            ObjObject[] objects = new ObjObject[objCount];
            for (int i = 0; i < objCount; i++)
            {
                objects[i].StartIndex = reader.ReadInt32();
                objects[i].EndIndex = reader.ReadInt32();
                objects[i].Name = reader.ReadString();
                objects[i].MaterialName = reader.ReadString();
            }

            int groupCount = reader.ReadInt32();
            ObjGroup[] groups = new ObjGroup[groupCount];
            for (int i = 0; i < groupCount; i++)
            {
                groups[i].StartIndex = reader.ReadInt32();
                groups[i].EndIndex = reader.ReadInt32();
                groups[i].Name = reader.ReadString();
                groups[i].MaterialName = reader.ReadString();
            }

            string mtllib = reader.ReadString();
            ...
            return new ObjData(vertices, indices, objects, groups, mtllib);
        }
